...
CoreFunc <- function(i){
  ...
  #write.csv(dhdt_param_mod, file = path_code&'\\Temp\\dhdt_mod'&i&'.csv')
  #write.csv(ID_fit + dhdt_param_mod*0, file = path_code&'\\Temp\\ID'&i&'.csv')
  #return(outPut)
}
...
if(file.exists(path&'\\ModelOutput\\results_distributedscale\\melt'&ModelRunSeries[i]&'.csv') == TRUE){
cit[i] <- 1
} else{cit[i]<-NA}
}
ModelRunSeries <- DEM_dataframe[which(!is.na(dhdt_res[])),1][which(is.na(cit))];
