...
#  library(lubridate)
  ...
  # Shortwave Radiation
  if(albedoDyn=='const'){
    EBCoreInput$SWout <- EBCoreInput$SWin * EBCoreInput$albedo_d
  }
  ...
  k_d <- vector()
  catchDepth <- matrix(0, nrow = datasize, ncol = 1)
  for(t in 1:datasize){
    if(t%%100==0){
  ...
  # switch between measured and modelled surface temperature
    switch(EBModType,
  ...
  # 'While' loop runs until successive calculated values are less than 0.01 C apart, or until the limit of "limitloop" (100 works well) iterations.
  # Additionally the loop breaks when the temperature gradient from surface to top layer is >= 450 degC/m, which happens in extreme cases of the 
  # iteration but generally results in the model spinning into non-convergence
  limitLoop <- 100
  if((abs(Ts[n]-T_d[t,1])/h)>=450){
    catchDepth[t] <- 1
  Ts[n] <- T_d[t,1] - 0.5
  }
  
  
  while (abs(Ts[n]-Ts[n-1]) > 0.01 && n<limitLoop && !is.na(Ts[n])&& abs(Ts[n]-T_d[t,1])/h<450){

    breakID <- 0
    stop = FALSE
  # Calculate temperature profile for the necessary values
  ...
  # Surface temperature iteration (Newton-Raphson method) calling subfunctions for all fluxes 
  ...
  if(n == limitLoop){T_s[t + 1] <- (Ts[99] + Ts[100]) / 2
  } else if(is.na(Ts[n])){
  ...
  # Calculate the final debris temperature profile
  ...
  CUMmelt <- cumsum(melt*(3600/timestep));
  },
  'clean' = {
  # (2) Model clean ice glacier
    ...
    #EBCoreInput$Snet <- EBCoreInput$SWin * 
